                        ...
                        <div class="row" style="margin-top: 10px;">
                            <div class="col-lg-8">
                                <div class="card">
                                    ...
                                    <div class="card-body">
                                        <form action="{{ route('menu-item-update') }}" method="post" style="position: relative;">
                                        {{ csrf_field() }}
                                            <input type="hidden" name="menu_id" value="{{ $menu->menu->id }}">
                                            <input type="hidden" name="page_id" id="pageId" value="{{ $menu_item->page_id }}">
                                            <input type="hidden" name="menu_item_id" value="{{ $menu_item->id }}">
                                            <div class="row">
                                                <div class="col-6">
                                                    <label for="x_card_code" class="control-label mb-1"> Name</label>
                                                    <div class="input-group">
                                                        <input name="menu_name" type="text" class="form-control" value="{{ $menu_item->name }}" id="pageName" required>
                                                    </div>
                                                </div>
                                                <div class="col-6">
                                                    <label for="x_card_code" class="control-label mb-1">URL</label>
                                                    <div class="input-group">
                                                        <input name="menu_url" type="text" class="form-control menu_url" value="{{ $menu_item->url }}" required>
                                                ...
                                                <div class="col-6"><br>
                                                    ...
                                                    <div class="input-group">
                                                        <select name="menu_target" class="form-control">
                                                           <option value="_blank" {{ $menu_item->target == "_blank" ? "selected" :"" }}>Blank</option>
                                                           <option value="_self" {{ $menu_item->target == "_self" ? "selected" :"" }}>Self</option>
                                                        </select>
                                                    </div>
                                                </div>
                                                <div class="col-6"><br>
                                                    ...
                                                    <div class="input-group">
                                                        <select name="menu_parent" class="form-control">
                                                            ...
                                                            @foreach( $menus as $item )
                                                                @if( $item->parent == 0 && $menu_item->id != $item->id)
                                                                    <option value="{{ $item->id }}" {{ $item->id == $menu_item->parent ? "selected" : "" }} >{{ $item->name }}</option>
                                                                @endif
                                                            @endforeach
                                                        </select>
                                                    </div>
                                            ...
                                            <div class="row">
                                                <div class="col-6"><br>
                                                    ...
                                                    <div class="input-group">
                                                        <input type="text" name="menu_class" class="form-control" value="{{ $menu_item->class_name }}" required>
                                                    </div>
                                                </div>
                                                <div class="col-6"><br>
                                                    <label for="x_card_code" class="control-label mb-1">ID</label>
                                                    <div class="input-group">
                                                        <input type="text" name="attr_id" class="form-control" value="{{ $menu_item->id_name }}" required>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="row">
                                                <div class="col-6"><br>
                                                    ...
                                                    <div class="input-group">
                                                        <label class="switch switch-3d switch-primary mr-3">
                                                            <input type="checkbox" name="enabled" class="switch-input" {{ $menu_item->enabled == 1 ? "checked" : "" }} >
                                            ...
                                            <div class="col-xs-3 float-left">
                                                <button id="payment-button" type="submit" class="btn btn-lg btn-info">Save</button>
                                            </div>
                                        </form>
                                        <form method="post" action="{{ route('menu-item-delete') }}">
                                            {{ csrf_field() }}
                                            ...
                                            <div class="col-xs-3 float-right">
                                                <button id="payment-button" type="submit" class="btn btn-lg btn-danger menuItem-delete">Delete</button>
                                            </div>
                                        </form>
                                    </div>
                                </div>
                ...
                <div class="modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="mediumModalLabel">Links</h5>
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $(".menuItem-delete").click(function(){
